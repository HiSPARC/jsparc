/* About: Version
   ...
   and http://www.opensource.org/licenses/mit-license.php for further 
   information.
   ...
   The function "showerEnergy(data)" is used to make a map with the stations 
   and the showerlocation for the HiSPARC-project. The shower can be dragged 
   across the map. The measured number of MIP's (minimal ionising particles) 
   of one station is used to calculate the number of MIP's of the other 
   stations. When the number of measured MIP's is equal to the the number of 
   calculated MIP's, a location of the shower and an energy of the shower have 
   been found.
 ...
 *
 *
 * Calculates geodetic distance between two points specified by latitude / 
 * longitude using Vincenty inverse formula for ellipsoids
 *
...
function distVincenty(latitude1, longitude1, latitude2, longitude2) {
    ...
    // note: to return initial/final bearings in addition to distance, use 
    // something like:
    //    var fwdAz = Math.atan2(cosU2*sinLambda,  
    //        cosU1*sinU2-sinU1*cosU2*cosLambda);
    //    var revAz = Math.atan2(cosU1*sinLambda, 
    //        -sinU1*cosU2+cosU1*sinU2*cosLambda);

    //  return {distance:s;initialBearing:toDeg(fwdAz);
    //          finalBearing:toDeg(revAz)};
}

// End of Vincenty Inverse Solution of Geodesics on the Ellipsoid (c) 
...
var proj4326 = new OpenLayers.Projection("EPSG:4326"); 
    // projection according WGS 1984
var projmerc = new OpenLayers.Projection("EPSG:900913"); 
    // projection according Mercator
...
function invNKG(shower4326, stationIndex, data) {
    ...
    if (data.events[stationIndex].latitude !== "") {
        r = distVincenty(shower4326.y, shower4326.x, 
                         data.events[stationIndex].latitude, 
                         data.events[stationIndex].longitude);}
    return S * Math.pow((r / r0), (alfa)) * Math.pow((1 + (r / r0)), 
                        (eta - alfa));
}
...
function invAgase(shower4326, stationIndex, data) {
    ...
    if (data.events[stationIndex].latitude !== "") {
        r = distVincenty(shower4326.y, shower4326.x, 
            data.events[stationIndex].latitude, 
            data.events[stationIndex].longitude);}
    return S * Math.pow((r / r0), (-1.2)) * Math.pow((1 + (r / r0)), 
                        (-2.64)) * Math.pow((1 + r * r / 1000000), (-0.6));
}

function NKG(pMerc, k, stationIndex, data) {
    var p4326 = (pMerc);
    var r = distVincenty(p4326.y, p4326.x, data.events[stationIndex].latitude, 
                         data.events[stationIndex].longitude);
    return k * Math.pow((r / r0), (-alfa)) * Math.pow((1 + (r / r0)), 
                        (alfa - eta));
}

function agase(pMerc, k, stationIndex, data) {
    var p4326 = (pMerc);
    var r = distVincenty(p4326.y, p4326.x, data.events[i].latitude, 
                         data.events[i].longitude);
    return k * Math.pow((r / r0), (1.2)) * Math.pow((1 + (r / r0)), (2.64)) * 
           Math.pow((1 + r * r / 1000000), (0.6));
}
...
function energy(k) {
    return 2.15e17 * Math.pow((k * Math.pow((600 / r0), 
           (-alfa)) * Math.pow((1 + (600 / r0)), (alfa - eta))), 1.015);
}
...
function calcError(htmlInfo, data) {
    ...
    $("#" + htmlInfo.stationEr).val(chiSq.toFixed(4)); result.error = chiSq;
/*    for (j = 0; j < 4; j++) {
        for (i = 0; i < data.events.length; i++) {
...
function calcEnergy(htmlInfo, showerMerc, data) {
    ...
    for (i = 0; i < data.events.length; i++) {
        $("#" + htmlInfo.mipCalcId + i).val(NKG(showerMerc, k, i, 
          data).toFixed(3));}
...
function sendResult() {
    ...
    $.getJSON(URL + 'result/', result, function(data) {
        ...
        // FIXME: This 'reload' should be smarter and remember the Title 
        // and PIN (possible probably also the Student name)
        window.location.reload();
    });
}

function transPlace(pMerc) {
    var p4326 = new OpenLayers.Geometry.Point(pMerc.geometry.x, 
                pMerc.geometry.y); // makes a temporary helppoint
...
function writeDist(htmlInfo, pMerc, data) {
    ...
    for (i = 0; i < data.events.length; i++) {
        $("#" + htmlInfo.distId + i).val(distVincenty(p4326.y, p4326.x, 
          data.events[i].latitude, data.events[i].longitude));}
}
...
function makeShowerMap(htmlInfo, data) { //htmlInfo and data are JSON's!
    ...
    for (i = 0; i < data.events.length; i++) {
        ...
        if (mapData.xmax < data.events[i].longitude){
            mapData.xmax = data.events[i].longitude;}
        if (mapData.xmin > data.events[i].longitude){
            mapData.xmin = data.events[i].longitude;}
        if (mapData.ymax < data.events[i].latitude){
            mapData.ymax = data.events[i].latitude;}
        if (mapData.ymin > data.events[i].latitude){
            mapData.ymin = data.events[i].latitude;}}
    ...
    map.setCenter(new OpenLayers.LonLat(4.950, 52.355).transform(proj4326, 
                  projmerc), 5); //shows the map

    var showerLayer = new OpenLayers.Layer.Vector("Shower"); 
        //makes a vectorlayer for the shower
    showerMerc = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(x,
                 y).transform(proj4326, projmerc),
            {some: 'data'},
    ...
    var stationLayer = new OpenLayers.Layer.Vector("Stations"); 
        //makes a vectorlayer for the stations
    for (i = 0; i < data.events.length; i++) {
        station = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(
                  data.events[i].longitude, data.events[i].latitude).
    ...
    var dragShower = new OpenLayers.Control.DragFeature(showerLayer); 
        // makes features in the vector-layer draggeble
    ...
    // writes the distances to the html-form
    map.events.register("mousemove", map, function (e) {
        writeDist(htmlInfo, showerMerc, data);}); 
        // calls writeDist() when the mouse moves
    map.events.register("touchmove", map, function (e) {
        writeDist(htmlInfo, showerMerc, data);}); 
        // calls writeDist() when the finger moves
}
...
function plotGraph(htmlInfo, data) {
    ...
    for (j = 0; j < data.events.length; j++) {
        for (k = 0; k < detNum[j]; k++) {
            tracedata[k] = [[(data.events[j].nanoseconds - tmin), 
                              data.events[j].traces[k][0]]];
            ...
            for (i = 1; i < data.events[j].traces[k].length; i++) {
                tracedata[k].push([(i * 2.5 + data.events[j].nanoseconds - 
                                    tmin), data.events[j].traces[k][i]]);}
            eventdata.push(tracedata[k]);}}

    var eventColors = [];
    var _eventColors = ["#600", "#f00", "#f90", "#ff0", "#6f0", "#6ff", 
                        "#f0f", "#ccc"]
    ...
    for (j = 0; j < data.events.length; j++) {
        ...
        var _tracePlotStyle = {
            ...
            axes: {
                xaxis: {
                    min: 0,
                    max: Math.ceil((data.events[j].traces[0].length * 2.5 + 
                    data.events[j].nanoseconds - tmin) / 40.0) * 40.0},
                yaxis: {
                    min: Math.ceil(trace_min * 1.1 / 2.0) * 2.0,
                    max: 0}}};
...
function toZenith(RA,Dec,Lon,Lat,ST){
    var sinDec = Math.sin(toRad(Dec)), cosDec = Math.cos(toRad(Dec)), 
        sinLat = Math.sin(toRad(Lat)), cosLat = Math.cos(toRad(Lat));
    var ha = ST - Lon - RA, cosZenith = (sinDec * sinLat + 
             cosDec * cosLat * Math.cos(toRad(ha))).toFixed(4);
    var zenith = toDeg(Math.acos(cosZenith)), azimuth;
    if (cosLat * Math.sin(toRad(zenith)).toFixed(4) == 0) {
        azimuth = 0;}
    else {azimuth = toDeg(Math.acos((sinDec - sinLat * cosZenith) / 
                    (cosLat * toRad(Math.sin(zenith))))).toFixed(4);}
...
function zenithData(data, star) {
    ...
    for (i = 0; i < data.events.length; i++) {
        mapData.longitude += data.events[i].longitude; 
        mapData.latitude += data.events[i].latitude;}
    var Lon = mapData.longitude / data.events.length, 
        Lat = mapData.latitude / data.events.length, ST = 0;
    for (i = 0; i < star.length; i++){
       for(j = 0; j < star[i].length; j++){
           var netCoordInput = toZenith(star[i][j][0], star[i][j][1], 
               Lon, Lat, (data.events[0].timestamp / 86400 - 10957));
...
function WGS84toECEF(latitude, longitude, altitude){
    ...
    y=(a+altitude)*Math.cos(latitude)*Math.sin(longitude), 
    z=(b+altitude)*Math.sin(latitude), 
    coordinate={"x":x,"y":y,"z":z,"latitude":latitude};
    return coordinate;
}
...
function netLoc(data, height){
    ...
    for(var i = 0; i < data.events.length; i++){
        netCoord.push(WGS84toECEF(data.events[i].latitude, 
                                  data.events[i].longitude, 
                                  (data.events[i].altitude+height)));
    }
    return netCoord;
}
...
function showerDirection(data){
    ...
    for(var i = 0; i< data.events.length; i++){
        ...
        showerDir.latitude += eval(netCoord[i].latitude);
    }
    ...
    showerDir.latitude = showerDir.latitude / data.events.length;
    norm = 1/(Math.sqrt(showerDir.x * showerDir.x + 
           showerDir.y * showerDir.y));
    showerDir.xLon = -showerDir.y*norm; showerDir.yLon = showerDir.x*norm; 
    showerDir.zLat = Math.cos(showerDir.latitude);
    showerDir.yLat = -showerDir.y*norm*Math.sin(showerDir.latitude); 
    showerDir.xLat = -showerDir.x*norm*Math.sin(showerDir.latitude);
    return showerDir;
}
...
function timeCalc(htmlInfo, data, dRA, dDec){ 
    ...
    /*
    * Uses a plane 20000m above the Earth's surface. This is the mean hight 
    * of first interactions.
    */

    var measurement = new Array, calculation = new Array, 
        netCoord = netLoc(data, 0), showerDir = showerDirection(data);
    var z = (showerDir.z+
            20000*showerDir.zLat*Math.tan(toRad(dDec))).toFixed(0);
    var y = (showerDir.y+20000*(showerDir.yLat*Math.tan(toRad(dDec))+
            showerDir.xLon*Math.tan(toRad(dRA)))).toFixed(0);
    var x = (showerDir.x+20000*(showerDir.xLat*Math.tan(toRad(dDec))+
            showerDir.yLon*Math.tan(toRad(dRA)))).toFixed(0);
    var dx = x-netCoord[0].x, dy = y-netCoord[0].y, dz = z-netCoord[0].z, 
        time = Math.sqrt(dx*dx+dy*dy+dz*dz)/c;
...
function makeStarMap(htmlInfo, star) {
    var starStyle = {
        ...
        seriesDefaults: {
            ...
            markerOptions: {shadow: false, size: 6, style: "filledCircle", 
                            lineWidth: 0}},
        series: [{markerOptions: {size: 2}}, {markerOptions: {size: 3}}, {
                  markerOptions: {size: 4}}, {markerOptions: {size: 5}},
                 {markerOptions: {size: 6}}, {markerOptions: {size: 7}}, {
                  markerOptions: {size: 8}}, {markerOptions: {size: 9}}],
        legend: {show: false},
        cursor: {tooltipLocation: 'se', zoom: false},
        axesDefaults: {
            min: -60, max: 60, numberTicks: 9, 
            labelRenderer: $.jqplot.CanvasAxisLabelRenderer,
            labelOptions: {textColor: '#222', enableFontSupport: true}, 
            tickRenderer: $.jqplot.CanvasAxisTickRenderer,
            tickOptions: {textColor: '#222', enableFontSupport: true, 
            showGridline: true, mark: 'outside', markSize: 4}},
        axes: {
            xaxis:{label: "delta Right Ascension [degrees]"},
            yaxis: {label: "delta Declination [degrees]"}},
        grid: {
            shadow: false, background: "#fff", gridLineWidth: 1, 
            gridLineColor: "#ddd", borderWidth: 1, borderColor: "#000"}};
...
function interactionTrace(data) {
    ...
    for (i = 0; i < data.events.length; i++) {
        coordinate = toOrthogonal(i, data.events[i].longitude, 
                     data.events[i].latitude, data.events[i].altitude);
        x[i] = coordinate.x;
        y[i] = coordinate.y;
        z[i] = coordinate.z;
        t[i] = data.events[i].nanoseconds / 1e9;}
    if (i = 3) {
        A = 2 * ((x[0] - x[1]) * (y[0] - y[2]) - (x[0] - x[2]) * 
            (y[0] - y[1]));
        B = 2 * ((x[0] - x[2]) * (z[0] - z[1]) - (x[0] - x[1]) * 
            (z[0] - z[2]));
        C = 2 * ((x[0] - x[1]) * (t[2] - t[0]) - (x[0] - x[2]) * 
            (t[1] - t[0])) * c * c;
        D = (x[0] - x[2]) * ((x[0] - x[1]) * (x[0] - x[1]) + 
            (y[0] - y[1]) * (y[0] - y[1]) + 
            (z[0] - z[1]) * (z[0] - z[1]) + 
            (t[0] - t[1]) * (t[0] - t[1]) * c * c) - 
            (x[0] - x[1]) * ((x[0] - x[2]) * (x[0] - x[2]) + 
            (y[0] - y[2]) * (y[0] - y[2]) + 
            (z[0] - z[2]) * (z[0] - z[2]) + 
            (t[0] - t[2]) * (t[0] - t[2]) * c * c);
        E = 2 * ((y[0] - y[1]) * (z[0] - z[2]) - 
        ...
        F = 2 * ((y[0] - y[2]) * (t[1] - t[0]) - 
            (y[0] - y[1]) * (t[2] - t[0])) * c * c;
        G = (y[0] - x[1]) * ((x[0] - x[2]) * (x[0] - x[2]) + 
            (y[0] - y[2]) * (y[0] - y[2]) + 
            (z[0] - z[2]) * (z[0] - z[2]) + 
            (t[0] - t[2]) * (t[0] - t[2]) * c * c) - 
            (y[0] - y[2]) * ((x[0] - x[1]) * (x[0] - x[1]) + 
            (y[0] - y[1]) * (y[0] - y[1]) + 
            (z[0] - z[1]) * (z[0] - z[1]) + 
            (t[0] - t[1]) * (t[0] - t[1]) * c * c);
        travelTime = Math.sqrt((x[0] - x[1]) * (x[0] - x[1]) + 
                               (y[0] - y[1]) * (y[0] - y[1]) + 
                               (z[0] - z[1]) * (z[0] - z[1]));
        travelTime = travelTime + Math.sqrt((x[1] - x[2]) * (x[1] - x[2]) + 
                               (y[1] - y[2]) * (y[1] - y[2]) + 
                               (z[1] - z[2]) * (z[1] - z[1]));
        travelTime = travelTime + Math.sqrt((x[0] - x[2]) * (x[0] - x[2]) + 
                               (y[0] - y[2]) * (y[0] - y[2]) + 
                               (z[0] - z[2]) * (z[0] - z[2]));
        travelTime = travelTime / (4 * c);
        travelTime = travelTime - t[0];
        alert(c * (travelTime + t[0]) + ", " + c * (travelTime + t[1]) + 
                   ", " + c * (travelTime + t[2]));
        ALFA = ((travelTime + t[0]) * (B * C + E * F) + B * D + E * G) / 
               (A * A + B * B + E * E);
        BETA = (D * D + G * G + 2 * (C * D + F * G) * (travelTime + t[0]) + 
               (C + F - c * c * A * A) * (travelTime + t[0]) * 
               (travelTime + t[0])) / (A * A + B * B + E * E);

        if ((ALFA * ALFA - BETA) < 0) {
            alert("No solution, D = " + (ALFA * ALFA - BETA) + ", ALFA = " + 
                  ALFA + ", BETA = " + BETA)}
        else {
